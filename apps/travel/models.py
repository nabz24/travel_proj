from __future__ import unicode_literals

from django.db import models
import re, bcrypt
import datetime

# Create your models here.
Email_REGEX =re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,3}$')
Name_REGEX = re.compile(r'^[A-Za-z]{3,30}$')
Password_REGEX = re.compile(r'^[A-Za-z0-9@#$%^&*+=]{8,}')
message_REGEX = re.compile(r'^[A-Za-z ]{1,}$')


class UsersManager(models.Manager):
    def register(self, postData):
        error_list = []
        cur_date = datetime.datetime.today().strftime('%Y-%m-%d')
        if not Name_REGEX.match(postData['name']):
            error_list.append("First name is not valid")
        if not Name_REGEX.match(postData['username']):
            error_list.append("Username not valid")
        if not Name_REGEX.match(postData['username']):
            error_list.append("username is not vaid")
        if not Password_REGEX.match(postData["password"]):
            error_list.append('Password does not match')
        if not postData['password'] == postData['conf_pass']:
            error_list.append('Password does not match confirm password')

        if len(error_list)<1:
            hashed_password = bcrypt.hashpw(postData['password'].encode(), bcrypt.gensalt())
            if self.filter(username = postData['username']).exists():
                error_list.append("username is already registered")
            new_user = self.create(name = postData['name'], username = postData['username'], password = hashed_password)
            user_id = self.get(username=postData['username']).id
            return{'success' : True, 'user_id' : user_id}
        else:
            return {'success' : False, 'errors' : error_list}

    def login(self, postData):
        error_l = []
        if Users.objects.filter(username = postData['log_username']).exists():
            password = postData['log_password'].encode()
            stored_hashed = Users.objects.get(username= postData['log_username']).password
            if bcrypt.hashpw(password, stored_hashed.encode()) != stored_hashed:
                error_l.append("incorrect password")
            else:
                user = Users.objects.get(username = postData['log_username'])
        else:
            error_l.append("Email does not exist!")
        if len(error_l) < 1:
            return {'success' : True, 'user_id' : user.id}
        else:
            print error_l
            return {'success' : False, 'errors' : error_l}

class TripManager(models.Manager):
    def quoteReg(self,postData):
        error_list = []
        cur_date = datetime.datetime.today().strftime('%Y-%m-%d')
        mess = postData['description']
        from_date = postData['start_date']
        end_date = postData['end_date']
        if not message_REGEX.match(postData['destination']):
            error_list.append("Destination field is invalid")
        if mess == "":
            error_list.append("message is empty")
        if not cur_date < from_date:
            error_list.append('Travel Date from is not valid')
        if not from_date < end_date:
            error_list.append('"Travel date to" is before "travel date from" ')
        if len(error_list)<1:
            # quote =
            new_quote = self.create(destination = postData['destination'], description = postData['description'], contributer_id = postData['user_id'], date_start = postData['start_date'], date_end = postData['end_date'])
            return{'success' :True}
        else:
            return{ 'success' :False, 'errors': error_list}







class Users(models.Model):
    name = models.CharField(max_length = 45)
    username = models.CharField(max_length = 45)
    password = models.CharField(max_length = 45)
    created_at = models.DateField(auto_now_add = True)
    objects = UsersManager()


class Trip(models.Model):
    contributer = models.ForeignKey(Users, related_name = "contrib")
    destination = models.CharField(max_length = 45)
    description = models.TextField()
    date_start = models.DateField()
    date_end = models.DateField()
    join = models.ManyToManyField(Users, related_name = "joins")
    objects = TripManager()

    # def __unicode__(self):
    #     return "id:" + str(self.id) + "destination:" + self.date_start
